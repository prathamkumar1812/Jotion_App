...
export const getById=query({
    ...
    handler:async(ctx,args)=>{
        ...
        if(!document){
          ...
          if(document.isPublished&&!document.isAttached){
              return document;
          }
            if(!identity){
                throw new Error("Unauthenticated")
...
export const getSearch=query({
    handler:async(ctx)=>{
        ...
        ).filter((q)=>q.eq(q.field("isAttached"),false)).order("desc").collect();
        return documents;
    }
});
...
export const getTrash=query({
    handler:async(ctx)=>{
        ...
        q.eq(q.field("isAttached"),true)).order("desc").collect();
        return documents;
